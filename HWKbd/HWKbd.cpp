...
void HWKbd::connect(int pKBDrst, int pKBDclk, int pKBDread, int pLEDrst, int pLEDclk, HWKbd_Encoder *enc){
  ...
  _prtKBD_reset = portOutputRegister(digitalPinToPort(pinKBD_reset));
  _prtKBD_clock = portOutputRegister(digitalPinToPort(pinKBD_clock));
  _prtKBD_read  = portInputRegister(digitalPinToPort(pinKBD_read));
  _prtLED_reset = portOutputRegister(digitalPinToPort(pinLED_reset));
  _prtLED_clock = portOutputRegister(digitalPinToPort(pinLED_clock));
#endif
  isConnected = true;
  setLEDs( 0);
#ifdef HWKBD_DEBUG_
